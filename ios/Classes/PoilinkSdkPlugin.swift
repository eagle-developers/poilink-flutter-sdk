...
class PoilinkHostApiImpl: PoilinkHostApi {
  ...
  private func buildWebPortalOptions(_ msg: WebPortalOptionsMsg) -> PoilinkSDK.WebPortalOptions {
    ...
    opts.onRewardReceive = { [weak self] grantId, itemCode, quantity, missionCode in
      self?.eventApi?.onRewardReceive(
        handle: handle, grantId: grantId, itemCode: itemCode, quantity: Int64(quantity), missionCode: missionCode) { _ in }
    }
    return opts
  }
}
